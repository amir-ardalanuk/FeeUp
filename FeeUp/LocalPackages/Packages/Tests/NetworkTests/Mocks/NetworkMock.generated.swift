...
// MARK: - DataDecoderProtocol

open class DataDecoderProtocolMock: DataDecoderProtocol, Mock {
    public init(sequencing sequencingPolicy: SequencingPolicy = .lastWrittenResolvedFirst, stubbing stubbingPolicy: StubbingPolicy = .wrap, file: StaticString = #file, line: UInt = #line) {
        SwiftyMockyTestObserver.setup()
        self.sequencingPolicy = sequencingPolicy
        self.stubbingPolicy = stubbingPolicy
    ...
    open func decode<T: Decodable>(_ data: Data, type: T.Type) throws -> T {
        addInvocation(.m_decode__datatype_type(Parameter<Data>.value(`data`), Parameter<T.Type>.value(`type`).wrapAsGeneric()))
		let perform = methodPerformValue(.m_decode__datatype_type(Parameter<Data>.value(`data`), Parameter<T.Type>.value(`type`).wrapAsGeneric())) as? (Data, T.Type) -> Void
		perform?(`data`, `type`)
		var __value: T
		do {
		    __value = try methodReturnValue(.m_decode__datatype_type(Parameter<Data>.value(`data`), Parameter<T.Type>.value(`type`).wrapAsGeneric())).casted()
		} catch MockError.notStubed {
			onFatalFailure("Stub return value not specified for decode<T: Decodable>(_ data: Data, type: T.Type). Use given")
			Failure("Stub return value not specified for decode<T: Decodable>(_ data: Data, type: T.Type). Use given")
		} catch {
		    throw error
		}
		return __value
    ...
    fileprivate enum MethodType {
        case m_decode__datatype_type(Parameter<Data>, Parameter<GenericAttribute>)

        static func compareParameters(lhs: MethodType, rhs: MethodType, matcher: Matcher) -> Matcher.ComparisonResult {
            switch (lhs, rhs) {
            case (.m_decode__datatype_type(let lhsData, let lhsType), .m_decode__datatype_type(let rhsData, let rhsType)):
				var results: [Matcher.ParameterComparisonResult] = []
        ...
        func intValue() -> Int {
            switch self {
            case let .m_decode__datatype_type(p0, p1): return p0.intValue + p1.intValue
            }
        }
        func assertionName() -> String {
            switch self {
            case .m_decode__datatype_type: return ".decode(_:type:)"
            }
        }
    }
    ...
    open class Given: StubbedMethod {
        ...
        public static func decode<T: Decodable>(_ data: Parameter<Data>, type: Parameter<T.Type>, willReturn: T...) -> MethodStub {
            return Given(method: .m_decode__datatype_type(`data`, `type`.wrapAsGeneric()), products: willReturn.map({ StubProduct.return($0 as Any) }))
        }
        public static func decode<T:Decodable>(_ data: Parameter<Data>, type: Parameter<T.Type>, willThrow: Error...) -> MethodStub {
            return Given(method: .m_decode__datatype_type(`data`, `type`.wrapAsGeneric()), products: willThrow.map({ StubProduct.throw($0) }))
        }
        public static func decode<T: Decodable>(_ data: Parameter<Data>, type: Parameter<T.Type>, willProduce: (StubberThrows<T>) -> Void) -> MethodStub {
            let willThrow: [Error] = []
			let given: Given = { return Given(method: .m_decode__datatype_type(`data`, `type`.wrapAsGeneric()), products: willThrow.map({ StubProduct.throw($0) })) }()
    ...
    public struct Verify {
        ...
        public static func decode<T>(_ data: Parameter<Data>, type: Parameter<T.Type>) -> Verify where T:Decodable { return Verify(method: .m_decode__datatype_type(`data`, `type`.wrapAsGeneric()))}
    }
    ...
    public struct Perform {
        ...
        public static func decode<T>(_ data: Parameter<Data>, type: Parameter<T.Type>, perform: @escaping (Data, T.Type) -> Void) -> Perform where T:Decodable {
            return Perform(method: .m_decode__datatype_type(`data`, `type`.wrapAsGeneric()), performs: perform)
        }
    }
